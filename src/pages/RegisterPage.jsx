...
export default function RegisterPage() {
    ...
    const passwordOk = user.password === user.confirmPassword && user.password.length > 5
    ...
    return (
        <div className="flex justify-center h-full">
            ...
            <form className="flex flex-col gap-4 my-6">
                ...
                {user.confirmPassword.length > 5 && user.password !== user.confirmPassword && ( 
                    <span className="text-red-400 text-sm m-0">Parolele nu coincid!</span> 
                )}
                ...
                <button
                    ...
                    className={`w-full mx-auto text-white py-2 mt-3 rounded-4xl shadow-md transition cursor-pointer 
                        ${passwordOk ? 'bg-(--primary) hover:bg-(--primary-darker)' : 'bg-gray-500 cursor-not-allowed opacity-80'}`}
                    onClick={(e) => {
                        e.preventDefault()
