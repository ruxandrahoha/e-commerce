            ...
            <div className="flex justify-between pt-4">
              <p className="text-lg mt-2">{product.price} lei</p>
                <Link to={`/dashboard/edit/${product.id}`}>
                  <button className="text-md bg-(--primary) rounded-4xl p-2 px-4 cursor-pointer text-(--secondary) hover:bg-(--primary) transition">Edit</button>
                </Link>
                <button className="text-md bg-(--primary) rounded-4xl p-2 px-4 cursor-pointer text-(--secondary) hover:bg-(--primary) transition" onClick={() => deleteProduct(product)}>Delete</button>
            </div>
        </div>
    )
  })
    ...
    <div className="flex flex-wrap justify-center items-center w-5/6 m-auto">
      <div className="w-64 h-96 flex justify-center items-center bg-white m-2 p-4 rounded-4xl shadow-md">
        <Link to="/dashboard/add">
          <button className="text-md h-12 my-auto bg-(--primary) rounded-4xl p-2 px-4 cursor-pointer text-(--secondary) hover:bg-(--secondary) active:bg-(--secondary) transition" onClick={() => addProduct()}>Add new item</button>
        </Link>
      </div>
      {cardElements}
    </div>
