...
import { listenToProducts, deleteProduct, addCategory, editCategoryName, deleteCategory } from "../firebase"
...
export default function Dashboard() {
  ...
  const categoryElements = categories.map(category => {
    return (
      <button
          ...
          className={`flex gap-2 px-4 py-2 rounded-4xl shadow-md text-(--secondary) cursor-pointer transition hover:bg-(--primary-darker) ${filter === category.name ? "bg-(--primary-darker) " : "bg-(--primary)"}`}
        >
          {category.name}
          {
  ...
  const cardElements = filteredProducts.map(product => {
    return (
        <div key={product.id} className="w-64 bg-white m-2 p-4 rounded-4xl shadow-md hover:shadow-lg/25">
